...
**Please note that if you are trying to compare with the results that we obtained in the manuscript, you will get different results if using a different Kraken2 version. We have asked about the reason for this on the [Kraken2 Github page](https://github.com/DerrickWood/kraken2/issues/594), but have not yet received a reply.** 
This seems to happen even when comparing between e.g. different conda files that contain the same Kraken2 version. It's therefore possible that you will see slightly different results even if you use the same version of Kraken2 that I did, and until the authors respond I don't have a good explanation for what the reason for this might be. I can only say that it still seems to do a reasonable job, even if the results do differ a bit between different versions.
...
```{bash, eval=FALSE}
mkdir metaphlan_out
mkdir metaphlan_out_no_reads
#parallel -j 1 'metaphlan {1} --input_type fastq -o metaphlan_out_no_reads/{1/.}.txt --bowtie2db metaphlan4/' ::: fasta_files/*
#for i in metaphlan_out_no_reads/* ; do mv $i "${i/.fastq/}" ; done
#note that I first ran the above, but had forgotten to add the rel_ab_w_read_stats option, so I reran it as follows:
...
#but for an actual comparison it should have been run like so:
parallel -j 1 'metaphlan {1} --input_type fastq -o metaphlan_out/{1/.}.txt --bowtie2db metaphlan4/ --nproc 12 -t rel_ab_w_read_stats' ::: fasta_files/*.txt
...
You should now have a summary plot called Comparison_of_databases.png that shows the performance of the RefSeq Complete V205 database, the original MetaPhlAn 3 run with estimated number of reads, and MetaPhlAn 4 with estimated number of reads. 

# Compare with a different tool

If you have the output from using a different tool, as long as it is in the expected format with NCBI species taxonomy ID's as rows and the sample names as columns then you should be able to run this like so:
```{bash, eval=FALSE}
conda activate compare
python compare_truth.py
python make_summary_plot.py
