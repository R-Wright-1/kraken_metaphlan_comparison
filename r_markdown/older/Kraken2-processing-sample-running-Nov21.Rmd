---
title: "Kraken2 and MetaPhlAn3 confidence and database testing - getting and running the sample Nov 21"
output:
  html_document:
    toc: yes
    toc_float: yes
...
concat_paired_end.pl -p 4 --no_R_match -o primate_cat_reads_fastqs kneaddata_out_primate/paired/*_paired_*.fastq 

```

### Combine all samples into one folder
...
for f in folders:
  if '16S' in f or 'all_samples' in f: continue
  if 'kneaddata' in f: continue
  files = os.listdir(folder+f)
  prefix = f.split('_')[0]
  for fi in files:
    command = 'mv '+folder+f+'/'+fi+' '+folder+out+prefix+'_'+fi
    os.system(command)
```

### 16S files

Copied across the .tsv feature tables and the rep_seqs/asv files from the picrust_manuscript folder of Gavin's.

Combine the sequences to one file:
```{bash, eval=FALSE}
cat *.f* > all_sequences.fna
```

Import into QIIME2 (with install of latest QIIME2 version first):
```{bash, eval=FALSE}
...
qiime tools import \
  --input-path all_sequences.fna \
  --output-path all_sequences.qza \
...
qiime feature-classifier classify-sklearn \
  --i-reads all_sequences.qza \
  --i-classifier /home/shared/taxa_classifiers/qiime2-2021.11_classifiers/silva-138-99-nb-classifier.qza \
  --p-n-jobs 12 \
  --output-dir taxa
```
...
Kraken:
```{bash, eval=FALSE}
sudo cp -a /home/storage/robyn/kraken2_databases/RefSeqV205_Complete_V2/ /scratch/ramdisk/
mkdir kraken2_outraw
mkdir kraken2_kreport

parallel -j 2 --progress 'kraken2 --use-names --threads 24 --db {3} --memory-mapping {1} --output kraken2_outraw/{1/.}.kraken --report kraken2_kreport/{1/.}.{2}.kreport --confidence {2}' ::: all_samples/* ::: 0.00 0.15 0.50 0.65 1.00 ::: /scratch/ramdisk/Kraken2_RefSeqCompleteV205/

parallel -j 48 'bracken -d {2} -i {1} -l S -o {1.}.bracken -r 150' ::: kraken2_kreport/*.kreport ::: /scratch/ramdisk/Kraken2_RefSeqCompleteV205/
mkdir kraken2_bracken_species
mv kraken2_kreport/*bracken* kraken2_bracken_species/

parallel -j 48 'bracken -d {2} -i {1} -l G -o {1.}.genus.bracken -r 150' ::: kraken2_kreport/*.kreport ::: /home/shared/Kraken2_RefSeqCompleteV205/
mkdir kraken2_bracken_genus
mv kraken2_kreport/*genus.bracken kraken2_bracken_genus/
...
# Get proportion of taxa covered by database

```{python, eval=FALSE}
#on vulcan
import pandas as pd
databases = ['kraken2_chocophlanV30-201901', 'Kraken2.0.8_Bracken150mer_RefSeqCompleteV93', 'RefSeqV205_Complete_V2', 'kraken2_RefSeqV208_nt']
direc = '/home/robyn/'
direc = '/home/robyn/databases_May2021/'
...
#need to change merged above so that it includes the parent tax id's 
...
for folder in ['kraken2_bracken_species/', 'kraken2_bracken_genus/']:
  #if 'species' not in folder: continue
  files = os.listdir(folder)
  files = [f for f in files if 'kreport' not in f]
  all_all_df = []
  all_df = []
  count = 0
  for fi in files:
    ...
    fn = fn[0]+'-'+fn[1]+'.'+fn[2]
    print(folder, fn, count)
    this_df = pd.DataFrame(pd.read_csv(folder+fi, index_col=[0,1], header=0, sep='\t').loc[:, 'new_est_reads']).rename(columns={'new_est_reads':fn})
  ...
  all_all_df = pd.concat([all_all_df, all_df]).fillna(value=0)
  all_all_df = all_all_df.groupby(by=all_all_df.index, axis=0).sum()
  all_all_df.to_csv(folder.replace('/', '')+'.csv')
```
...
## Remove taxa that aren't bacteria
