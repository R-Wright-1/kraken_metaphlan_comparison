...
parallel -j 36 'bracken -d {2} -i {1} -l S -o {1.}.bracken -r 150' ::: kraken2_kreport/RefSeqCompleteV205_100GB/*.kreport ::: /home/storage/robyn/kraken2_databases/RefSeqV205_Complete_V2_100GB/
mkdir kraken2_bracken_species/RefSeqCompleteV205_100GB/
mv kraken2_kreport/RefSeqCompleteV205_100GB/*bracken* kraken2_bracken_species/RefSeqCompleteV205_100GB/
...
parallel -j 2 --progress 'kraken2 --use-names --threads 18 --db {3} --memory-mapping {1} --output kraken2_outraw/chocophlan/{1/.}.kraken --report kraken2_kreport/chocophlan/{1/.}.{2}.kreport --confidence {2}' ::: all_samples/* ::: 0.05 0.10 0.15 0.20 0.25 0.30 0.35 0.40 0.45 0.50 0.55 0.60 0.65 0.70 0.75 0.80 0.85 0.90 0.95 1.00 0.00 ::: /scratch/ramdisk/kraken2_chocophlanV30-201901/
...
#folders = ['/home/robyn/simulated_samples/CAMI2/kraken2_outraw/GTDBr202RefSeqV205/',]
...
save_folder = '/home/robyn/simulated_samples/CAMI2/summary_outraw/'
...
this_folder = '/home/robyn/simulated_samples/CAMI2/kraken2_outraw/'+folders[0]
files = sorted(os.listdir(this_folder))
files = [this_folder+f for f in files]
files = files[:6]
```
...
The samples that had issues when I first ran this only had issues with ~5-10 reads each (out of >10 million), so I'm just skipping these - they tended to be that the line was not fully written and then there was another read there too so it didn't work well with the script. Not sure what these should be, I could re-run them but it is so few reads.

For the chocophlan samples:
mouse_sample_10.kraken, strain_sample_90.kraken, mouse_sample_33.kraken, mouse_sample_34.kraken, mouse_sample_52.kraken, strain_sample_61.kraken, strain_sample_26.kraken, plant_sample_7.kraken, marine_sample_9.kraken, mouse_sample_59.kraken, mouse_sample_38.kraken, mouse_sample_30.kraken

RefSeqCompleteV205:
mouse_sample_24.kraken, mouse_sample_10.kraken, strain_sample_97.kraken, strain_sample_99.kraken, strain_sample_91.kraken, strain_sample_93.kraken, strain_sample_95.kraken

Minikraken:
strain_sample_84.kraken, mouse_sample_61.kraken, plant_sample_1.kraken, mouse_sample_60.kraken, plant_sample_12.kraken, mouse_sample_26.kraken, mouse_sample_16.kraken, marine_sample_0.fq.kraken

RefSeqCompleteV205_100GB
strain_sample_85.kraken


I think I'll need to go back to these as it looks like the files themselves have problems, so I'll just rerun with the databases at the end.
...
## Run Kraken and output confidence

### Edit the src/classify.cc file

I think I should just fork the github repo and do it this way - so I've edited the classify.cc script
```{bash, eval=FALSE}
conda create --name kraken2-confidence-github
conda activate kraken2-confidence-github
mkdir tools/kraken2-confidence-github
./install_kraken2.sh /home/robyn/tools/kraken2-confidence-github/
```
The easiest thing looked like just printing the taxid, score and number of minimizers for each read processed, so I'll just save the output to a file.

### Test with MiniKraken V2

```{bash, eval=FALSE}
#might be issues with because there was some error often about children
#done
sudo cp -a /home/storage/robyn/kraken2_databases/minikraken2_v2_8GB_201904_UPDATE/ /scratch/ramdisk/
mkdir testing_confidence/
mkdir testing_confidence/all_samples/
cp all_samples/marine_sample_0.fq testing_confidence/all_samples/
cd all_samples/
mkdir kraken2_kreport
mkdir kraken2_outraw
mkdir kraken2_kreport/minikraken/
...
/home/robyn/tools/kraken2-confidence-github/kraken2 --use-names --threads 6 --db /scratch/ramdisk/minikraken2_v2_8GB_201904_UPDATE/ --memory-mapping all_samples/marine_sample_0.fq --output kraken2_outraw/minikraken/marine_sample_0.kraken --report kraken2_kreport/minikraken/marine_sample_0.0.00.kreport --confidence 0.00 > test_marine_sample_0.txt
```

This does give the scores output (I've added the taxonomy ID as well as the confidence score and total number of minimizers), but the issue is that it appears to be in a different order to how the sequences appear in the file, and if I print the sequence ID then this also doesn't seem to correspond to the correct taxonomy ID or minimizer counts.


Ideas:
- Run through and match each read with the correct classification - calculate the confidence of that classification and we can compare the confidence of true vs false classifications, or look at how far away the real classification is.
- Use the Otus for the binning project? The best binning would group all reads that originate from the same genome, so knowing the number of genomes that there should be will allow easy assessment of any of these methods.
- Look at proportion of each taxid that are correctly classified within samples (datasets?)